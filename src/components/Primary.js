...
{
    return <div class="badge badge-info text-lg text-black ">
                <img className="w-8 h-8 pr-1" src={"assets/Icons/"+tag+".svg"}/>
                {tag}
            </div>
}


function EmploymentRole({json,size_in,colour_in})
{
    const size  =size_in || "w-full h-full" 
    const colour=colour_in || ""; 
    return  <div class={size +" " +colour_in}>
                {JSON.stringify(json)}
            </div>  
}
...
export class HeroBanner extends Component
{
  ...
  render()  {
    ...
    // const test = cv.Mat((100,200),cv.CV_8UC3);
    // console.log(test);
